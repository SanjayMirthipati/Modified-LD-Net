...
def image_haze_removal(input_image, model):
    # Convert input image to hazy tensor
    hazy_image = transforms.ToTensor()(input_image).unsqueeze(0)
    ...
    return dehaze_image, time_taken
...
def calculate_ssim(image1, image2):
    ...
    score = ssim(array1, array2, multichannel=True)
...
def single_dehaze_test(input_path, ground_truth_path, model, epoch):
    # Read input image using OpenCV
    ...
    # Perform dehazing
    dehaze_image, time_taken = image_haze_removal(input_image, model)
    ...
    # Calculate SSIM
    ssim_score = calculate_ssim(dehaze_image, gt_image)

    # Log SSIM, PSNR, and time taken to dehaze into CSV file
    with open('dehaze_metrics_Original.csv', 'a', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([epoch, ssim_score, psnr, time_taken])
...
input_path = '/content/drive/MyDrive/archive/haze/0473_0.9_0.2.jpg'
ground_truth_path = '/content/drive/MyDrive/archive/clear_images/0473.jpg'
print("LightDehaze_Net_Original")
for i in range(0, 10):
    model = LightDehaze_Net()
    model.load_state_dict(torch.load('/content/drive/MyDrive/Classroom/New_weights_10K/Epoch' + str(i) + '.pth'))
    model.eval()
    single_dehaze_test(input_path, ground_truth_path, model, epoch=i+1)

    # Read metrics from CSV file
    with open('dehaze_metrics_Original.csv', 'r') as file:
        reader = csv.reader(file)
        for row in reader:
            epochs.append(int(row[0]))
            ssim_scores.append(float(row[1]))
            psnr_scores.append(float(row[2]))
            time_taken_list.append(float(row[3]))
...
# Plot SSIM
plt.subplot(3, 1, 1)
plt.bar(epochs, ssim_scores, color='b', alpha=0.7)
plt.xlabel('Epoch')
plt.ylabel('SSIM')
...
# Plot PSNR
plt.subplot(3, 1, 2)
plt.bar(epochs, psnr_scores, color='r', alpha=0.7)
plt.xlabel('Epoch')
plt.ylabel('PSNR')
plt.title('PSNR vs Epoch')

# Plot Time taken to dehaze
plt.subplot(3, 1, 3)
plt.bar(epochs, time_taken_list, color='g', alpha=0.7)
plt.xlabel('Epoch')
plt.ylabel('Time taken to dehaze (s)')
plt.title('Time taken to dehaze vs Epoch')
